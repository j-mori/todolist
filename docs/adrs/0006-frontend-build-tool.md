# ADR-0006: Vite + React 19 for the frontend
...
## Decision
Use **Vite (latest stable)** with `@vitejs/plugin-react` for the SWC-based React Fast Refresh. React **19.x** for the runtime. JSX runtime is automatic (`jsx: react-jsx`). No router (single-screen app); add one only if a routing need surfaces.
...
- **Follow-up:** If routing ever becomes nontrivial, evaluate `@tanstack/react-router` (type-safe) before reaching for `react-router`.
