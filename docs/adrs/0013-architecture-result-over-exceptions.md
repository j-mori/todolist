# ADR-0013: `Result<T, E>` and tagged-union errors over thrown exceptions
...
- **Positive:**
  ...
  - Mapping to HTTP is a `switch` on `error.kind` (`ValidationError → 400`, `TaskNotFound → 404`). No global error middleware required.
