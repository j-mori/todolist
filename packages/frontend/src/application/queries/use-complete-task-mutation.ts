...
export const useCompleteTaskMutation = (): UseMutationResult<
  Task,
...
> => {
  ...
  return useMutation<Task, ApiClientErrorException, Vars, Context>({
    mutationFn: async ({ id }) => unwrap(await tasksApi.complete(id)),
    onMutate: async ({ id }) => {
      const { previous } = await cancelAndSnapshotList(queryClient);
      const now = new Date().toISOString();
      writeOptimisticList(queryClient, (current) =>
        current.map((task) =>
          task.id === id ? { ...task, status: 'completed', updatedAt: now } : task,
        ),
      );
      return { previous };
    },
